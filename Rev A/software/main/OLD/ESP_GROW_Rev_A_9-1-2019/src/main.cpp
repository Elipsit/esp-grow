...
*
*8/04/2019
*Debug Mode added; press the screen when booting and it will bypass wifi connect and show sensors
*Wifi Status mode added; press the screen during normal operation to see wifi settings for 5 sec
*
****Status***
...
void checksensor(){
soilval =  map(analogRead(0),530,830,100,0);
...
void debug (){
  ...
  //soilval =  map(analogRead(0),480,830,100,0);
  delay(dht.getMinimumSamplingPeriod());
   h = dht.getHumidity();
   t = dht.getTemperature();
...
//debug feature to display what SSID and Pass is connected
void wifiStatus(){
  display.clearDisplay();
  display.display();
  delay(100);
